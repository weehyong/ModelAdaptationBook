...
================================================================================
    ...
    (book-conventions-aligned version of Weehyong's reference implementation)

    Original at: code/chapter02/ch02-openfinance-finetuning.py
                 https://github.com/weehyong/LLM-finetuning/blob/main/ch02-openfinance-finetuning.py

    This file is identical in logic to the original. It differs only in:
      - Filename uses underscores so `python -m chapter02.ch02_openfinance_finetuning`
        works without shell quoting.
      - Top-level executable code is wrapped in def main() so the module is
        importable without triggering training. Functions defined inside the
        original's executable flow (make_chat_message, apply_chat_template,
        generate_response) become nested functions inside main(); they
        capture model/tokenizer via closure.
      - Output path now follows the book convention
        (chapter02/runs/finance_lora/ instead of ./qwen-open-finance-output).

    All other prose (the seven-step tutorial, the design-decision rationale,
    the dataset notes) is preserved verbatim from Weehyong's original.
...
class Config:
    ...
    # -- Output --
    output_dir: str = "chapter02/runs/finance_lora"
...
def main():
    """Run the full Open Finance fine-tuning recipe end to end.
    
    See module docstring for what this does. Run with:
    
        python -m chapter02.ch02_openfinance_finetuning
    
    Requires CUDA. See code/chapter02/README.md for the install step.
    """
    config = Config()
